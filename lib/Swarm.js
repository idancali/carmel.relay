...
const LOG = debug_1.default('carmel:swarm');
class Swarm {
    constructor(server) {
        this._server = server;
    }
    ...
    async start() {
        this._set = util_1.promisify(this.server.pub.hset).bind(this.server.pub);
        this._get = util_1.promisify(this.server.pub.hgetall).bind(this.server.pub);
        this._del = util_1.promisify(this.server.pub.hdel).bind(this.server.pub);
    }
    async addresses() {
        try {
